  ...
  return (
    <div className='flex flex-wrap flex-shrink gap-2'>
        {filteredData.map((ext) => (
            <div key={ext.id} className={`${theme? 'bg-neutral-800' : 'bg-white'} p-2 rounded-xl min-w-[280px] flex-1  border border-white/10 shadow-md`}>
                ...
                <div className='flex justify-between items-center'>
                    <button onClick={() => handleDelete(ext.id)} className={`${theme? 'text-white border-white/20' : 'text-neutral-900 border-neutral-900/20'} px-4 py-1.5 border rounded-full text-[14px]`}>Remove</button>
